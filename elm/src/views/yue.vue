<template>
	<div>
		<h_nav>
			<div slot="left">
					<i class="iconfont icon-toright" @click="go"></i>
			</div>
			<div slot="title">
				我的余额
			</div>
		</h_nav>
		<div class="countent">
			<div class="top">
				<div class="to-cen">
					<div class="top-c-c">
						<p style="font-size: 0.55rem;">
							<span>当前余额</span>
							<a style="float: right;color: #3190e8;" href="#/yue/yuc">余额说明</a>
							<img style="float: right;margin-right: 8px;width: 0.6rem;height: 0.6rem;" src="data:image/png;base64," alt="">
							</p>
						<p style="font-size: 1.8rem;color: #333;">0.00<span>元</span></p>
						<div class="tx">提现</div>
					</div>
				</div>
			</div>
			<div class="tit">交易明细</div>
			<div class="jl">
				<img style="margin: 0 auto;display: block;width:8rem;height: 5rem;" src="data:image/png;base64," alt="">
				<p style="text-align: center;color: #666;margin-top: 20px;font-size: 0.7rem;">暂无明细记录</p>
			</div>
		</div>
		<router-view></router-view>
	</div>
</template>

<script>
import h_nav from './../components/h_nav.vue'
export default {
	components:{
		h_nav
	},
	methods:{
		go(){
			this.$router.go(-1)
		},
	}
}
</script>
<style>
	.jl{
		width: 16rem;
		height: 6.675rem;
	}
	.tit{
		width: 16rem;
		height: 2rem;
		padding-left: 1rem;
		box-sizing: border-box;
		color: #999;
		line-height: 2rem;
		font-size: 0.6rem;
	}
	*{
		margin: 0;
		padding: 0;
	}
	.tx{
		width: 14.6rem;
		height: 2rem;
		text-align: center;
		color:#fff;
		line-height: 2rem;
		background-color: #ccc;
		margin-top: 1rem;
	}
	
	.countent {
		width: 100%;
		height: auto;
	}
	.top{
		width: 16rem;
		height: 7.3rem;
		box-sizing: border-box;
		padding: 0.3rem;
		background-color: #3190e8;
	}
	.to-cen{
		box-sizing: border-box;
		width: 15.4rem;
		height: 6.7rem;
		padding: 0.375rem;
		background-color: #fff;
	}
	.top-c-c{
		width: 14.7rem;
		height: 5.875rem;
	}
	
</style>
