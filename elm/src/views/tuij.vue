<template>
	<div style="position: fixed;top: 0;z-index: 9;width: 100%;background: #f5f5f5;min-height: 100%;">
		<h_nav>
			<div slot="left">
					<i class="iconfont icon-toright" @click="go"></i>
			</div>
			<div slot="title">
				推荐有奖
			</div>
		</h_nav>
		<div class="j_ikl">
		<img src="http://f.hiphotos.baidu.com/image/h%3D300/sign=d985fb87d81b0ef473e89e5eedc551a1/b151f8198618367aa7f3cc7424738bd4b31ce525.jpg" alt=""/>
			</div>
		<div class="j_op">
			<div @click="fn">
				<img src="data:image/png;base64," alt="" />
				<p>邀请微信好友</p>
			</div>
			<div @click="fn">
				<img src="data:image/png;base64," alt="" />
				<p>邀请qq好友</p>
			</div>
			<div @click="fn">
				<img src="data:image/png;base64," alt="" />
				<p>面对面邀请</p>
			</div>
		</div>
		<div class="j_kl">
			<div>
				<p>累计收益</p>
				<p><span>0</span>元</p>
			</div>
			<div>
				<p>成功邀请</p>
				<p><span>0</span>人</p>
			</div>
		</div>
		<p style="text-align: center;color: #666;font-size: 0.4rem;margin-top: 0.75rem;">-收益说明-</p>
		<img style="width: 1.3rem;height: 1.6rem;display: block;text-align: center;margin: 0.75rem auto 0;" src="data:image/png;base64," alt="" />
		<p style="text-align: center;font-size: 0.4rem;color: #999;margin-top: 0.5rem;">还不赶紧取邀请好友</p>
	</div>
</template>

<script>
	import h_nav from './../components/h_nav.vue'
export default {
	components:{
		h_nav
	},
	methods:{
		go(){
				this.$router.go(-1)
		},
		fn(){
			alert('请在饿了么APP中打开')
		}
	},
}
</script>

<style>
	.j_ikl img{
		width: 100%;
    height: 10rem;
	}
	.j_op{
		display: flex;
		padding-top: 0.75rem;
		background: #FFFFFF;
		padding-bottom: 0.75rem;
	}
	.j_op div{
		flex: 1;
		text-align: center;
	}
	.j_op div img{
		width: 2.5rem;
    height: 2.5rem;
	}
	.j_op div p{
		font-size: .5rem;
    color: #333;
	}
	.j_kl{
		display: flex;
		margin-top: 2rem;
	}
	.j_kl div{
		flex: 1;
		text-align: center;
	}
	.j_kl div p{
		color: #666;
		font-size: 0.45rem;
	}
	.j_kl div:nth-of-type(1){
		border-right: solid 0.025rem #C4C4C4;
	}
	.j_kl div:nth-of-type(1) p span{
		font-size: .8rem;
    color: #ff5633;
    font-weight: 700;
	}
	.j_kl div:nth-of-type(2) p span{
		font-size: .8rem;
    color: #666;
    font-weight: 700;
	}
</style>