<template>
	<div style="background:#f4f4f4">
		<h_nav>
			<div slot="left">
					<i class="iconfont icon-toright" @click="go"></i>
			</div>
			<div slot="title">
				会员中心
			</div>
		</h_nav>
		<div style="min-height:50px">
			<div class="z_vip_top">
				<span>为账户</span>
				<span class="z_vip_span">{{names}}</span>
				<span>购买会员</span>
			</div>
			<!--会员特权-->
			<div class="z_vip-t">
				<div class="z_vip_c">
					<c_t>
						<span class="z_vip_c_span1" slot="vip-1">会员特权</span>
						<span class="z_vip_c_span2" slot="vip-2" @click="fn">会员说明</span>
						<span class="z_vip_c_span3" slot="vip-3">></span>
					</c_t>
				</div>
				<div class="z_vip_b">
					<div class="z_vip_img">
						<img src="data:image/jpeg;base64,">
					</div>
					<div class="z_vip_r">
						<p class="z_vip_r_p1">减免配送费</p>
						<p class="z_vip_r_p2">每月减免30单，每日可减免3单，每单最高减4元蜂鸟专送专享</p>
					</div>
				</div>
				<!---->
				<div class="z_vip_b">
					<div class="z_vip_img">
						<img src="data:image/jpeg;base64,">
					</div>
					<div class="z_vip_r">
						<p class="z_vip_r_p1">减免配送费</p>
						<p class="z_vip_r_p2">每月减免30单，每日可减免3单，每单最高减4元蜂鸟专送专享</p>
					</div>
				</div>
				<!---->
			</div>
			<!--开通会员-->
			<div class="z_vip_kt">
				<div class="z_vip_kt_t">
					<c_t>
						<span class="z_vip_c_span1" slot="vip-1">开通会员</span>
					</c_t>
				</div>
				<div class="z_vip_kt_b">
					<c_t>
						<span class="z_vip_c_span1" slot="vip-1">1个月</span>
						<span class="z_vip_c_span4" slot="vip-4">￥20</span>
						<p class="z_vip_c_p" slot="vip-5" @click="buy">购买</p>
					</c_t>
				</div>
			</div>
			<!--兑换会员-->
			<div class="z_vip_c_dh" @click="duih">
				<c_t>
					<span class="z_vip_c_span1" slot="vip-1">兑换会员</span>
					<span class="z_vip_c_span2" slot="vip-2">使用卡号卡密</span>
					<span class="z_vip_c_span3" slot="vip-3">></span>
				</c_t>
			</div>
			<!---->
			<!--购买记录-->
			<div class="z_vip_c_dh" @click="jil">
				<c_t>
					<span class="z_vip_c_span1" slot="vip-1">购买记录</span>
					<span class="z_vip_c_span2" slot="vip-2">开发票</span>
					<span class="z_vip_c_span3" slot="vip-3">></span>
				</c_t>
			</div>
			<!---->
		</div>
		<router-view></router-view>
	</div>
</template>

<script>
	import c_t from "../components/c_vip"
	import h_nav from './../components/h_nav.vue'
	export default {
		components: {
			c_t,h_nav
		},
		data() {
			return {
				names: localStorage.name
			}
		},
		methods:{
		go(){
				this.$router.go(-1)
		},
		fn(){
			location.href='#/vip/vip_s'
		},
		buy(){
			location.href = '#/vip/buy'
		},
		duih(){
			location.href = '#/vip/duip'
		},
		jil(){
			location.href = '#/vip/jilu'
		}
	},
	}
</script>

<style scoped>
	* {
		margin: 0;
		padding: 0;
		box-sizing: border-box;
	}
	
	.z_vip_top {
		width: 100%;
		height: 2.5rem;
		background: #f4f4f4;
		padding-left: .5rem;
	}
	
	.z_vip_top span {
		line-height: 2.5rem;
		font-size: .625rem;
	}
	
	.z_vip_span {
		color: #000;
		font-weight: bold;
	}
	
	.z_vip-t {
		width: 100%;
		/* background: red; */
	}
	
	.z_vip_c {
		width: 100%;
		height: 2rem;
		background: white;
		overflow: hidden;
	}
	
	.z_vip_b {
		width: 100%;
		height: 3rem;
		background: white;
		overflow: hidden;
		/* margin-top: .5rem; */
		padding-top: .5rem;
	}
	
	.z_vip_img {
		width: 2rem;
		height: 2rem;
		/* margin-top:.5rem; */
		/* background: blue; */
		float: left;
		margin-left: .5rem;
		margin-right: .5rem;
	}
	
	.z_vip_img img {
		width: 100%;
		height: 100%;
	}
	
	.z_vip_r {
		float: left;
		width: 12rem;
		height: 3rem;
	}
	
	.z_vip_r_p1 {
		font-size: .7rem;
	}
	
	.z_vip_r_p2 {
		font-size: .5rem;
		width: 10rem;
		color: #cccccc
	}
	
	.z_vip_kt {
		width: 100%;
		height: 4.5rem;
		background: white;
		margin-top: .5rem;
		overflow: hidden;
	}
	
	.z_vip_kt_t {
		width: 100%;
		height: 2rem;
		border-bottom: solid 1px #f4f4f4;
	}
	
	.z_vip_kt_b {
		width: 100%;
		height: 2rem;
		line-height: 2rem;
	}
	
	.z_vip_c_p {
		width: 2.5rem;
		height: 1.2rem;
		border: solid orange 1px;
		float: right;
		font-size: .6rem;
		margin-right: .5rem;
		text-align: center;
		line-height: 1.2rem;
		border-radius: .125rem;
		margin-top: .5rem;
	}
	
	.z_vip_c_dh {
		margin-top: .5rem;
		height: 2rem;
		background: white;
		margin-bottom: .5rem;
	}
</style>